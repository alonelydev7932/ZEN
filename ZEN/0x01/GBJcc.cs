...
namespace GBJcc
{
    public class Api
    {
        ...
        {
            if (vYFEC.whLOn.response.message == "invalidver")
            {
                if (!string.IsNullOrEmpty(vYFEC.whLOn.app_data.downloadLink))
                {
                    WebClient webClient = new WebClient();

                    string new_file = ZEN._0x02.mcyAy.RandomString(5);

                    webClient.DownloadFile(vYFEC.whLOn.app_data.downloadLink, Application.StartupPath + "\\"  + new_file + ".exe");

                    Process.Start(Application.StartupPath + "\\" + new_file + ".exe");
